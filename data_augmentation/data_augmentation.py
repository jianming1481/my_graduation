...
def rotate_translate_scale(ori_item):
    ...
    shift_x = random.randint(-1*max_cols/10,max_cols/10)
    shift_y = random.randint(-1*max_rows/10,max_rows/10)
    ...
    tmp_obj_img = cv2.warpAffine(ori_item, M_shift, (max_cols, max_rows))

    # Build Rotate Matrix
    M = cv2.getRotationMatrix2D((cols/2,rows/2),rot,scale)
    
    # Rotate Image
    tmp_obj_img = cv2.warpAffine(tmp_obj_img,M,(max_cols,max_rows))
...
def main():
    ...
    # Make labeled image be visualed
    visualiz_number = 1
    ...
    while total_list_num!=0:
        ...
        while len(rgb_image_list)!=0:
            ...
            """
            # Doing GrabCut Algorithm
            # Extract obj from shadow and make a label image
            rect = find_ROI(label_img)
            rgb_img = extract_from_shadow(my_grab_cut(ori_img,rect))
            rgb_obj_list.append(rgb_img)
