...
int main (int argc, char** argv)
{
  std::string fileName;
  if (argc != 2) {
    ROS_ERROR("rosrun trainning_data_builder pcd_reader <path/to/fileName>");
    ROS_ERROR("e.g. rosrun trainning_data_builder pcd_reader /home/user/PcdAndImg_fileName");
    return 0;
  }
  if (std::string(argv[1]) == "-h" || std::string(argv[1]) == "--help") {
    std::cout<<"Publish pcd and jpg file to topic"<<std::endl<<std::endl;
    std::cout<<"rosrun trainning_data_builder trainning_data_builder_node <path/to/fileName>"<<std::endl;
    std::cout<<"e.g. rosrun trainning_data_builder trainning_data_builder_node /home/user/PcdAndImg_fileName"<<std::endl;
    return 0;
  }
  fileName = argv[1];
  ...
  // std::stringstream ss;
  // ss << ros::package::getPath("trainning_data_builder") << "/pattern/" << "apple_00001" << ".pcd";
